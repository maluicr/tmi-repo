...
library(leaflet) # presentation with mapview
...
### Risco relativo (RR) {#sec-MetodosRR}
...
### Modelo de regressão Bayesiana {#sec-MetodosModelosBayes}

A partir de uma abordagem Bayesiana, o modelo de regressão espaço-temporal usado neste estudo é um modelo aditivo generalizado com distribuição de Poisson e função de ligação canónica (log):
...
Apesar de ter sido implementada uma análise de sensibilidade onde se especificaram e avaliaram diferentes distribuições à priori (priors) para a variância, as especificações apresentadas por baixo apenas incluem os resultados finais dessa análise.
...
$$
\beta_{EDI} \sim N(`r control.fixed$mean`, `r 1/control.fixed$prec`)
$$
...
Para a componente espacial ($u_i+v_i$) adoptou-se o modelo BYM2 (Besag-York-Mollié) especificado com uma combinação de *priors* com componente estruturada, $u_i$, e componente não-estruturada,  $v_i$, onde $u_i$ segue um modelo condicional autoregressivo intrinseco (iCAR) e $v_i$ segue uma distribuição gaussiana de média nula e variancia unitária.

Este modelo reparametriza esta soma em termos de uma precisão marginal e um parâmetro de mistura que controla a proporção da variância atribuída à componente estruturada. Para o desvio padrão marginal desta compontente,$\sigma_{u+v}$, especificou-se uma PC prior de modo a satisfazer a condição seguinte:

$$
P(\sigma_{u+v}>`r hyper.bym2$prec$param[1]`)= `r hyper.bym2$prec$param[2]`
$$

ou seja, com probabilidade 1% do desvio-padrão ser maior que 0.5. Esta especificação favorece uma baixa variabilidade espacial no contexto do RR.

Para o parâmetro de mistura do modelo BYM2, $\rho$ especificou-se a PC prior que reflete a convição à priori de que a heterogeneidade espacial não-estruturada é mais importante que os padrões de dependência espacial na explicação da variabilidade do RR entre concelhos (60% de probabilidade a priori de que menos de 50% da variância espacial seja explicada pela componente estruturada).

$$
P(\rho> `r hyper.bym2$phi$param[1]`)=`r hyper.bym2$phi$param[2]`
$$

Para a estrutura temporal especificou-se um passeio aleatório (*random walk*, RW1) de primeira ordem com erros aleatórios, $\epsilon$, gaussianos com média nula e variância unitária:

$$
\gamma_t = \gamma_{t-1}+ \epsilon_t
$$

Para o desvio padrão marginal das diferenças temporais, especificou-se a PC prior:

$$
P(\sigma_{\gamma}>`r hyper.temporal$prec$param[1]`)=`r hyper.temporal$prec$param[2]`
$$

Esta especificação favorece uma evolução temporal suave, com apenas 1% de probabilidade *a priori* de o desvio padrão das inovações exceder 0.1.

## Resultados {#sec-Resultados}

### Taxa de mortalidade infantil (TMI) {#sec-ResultadosTMI}

A TMI anual observada foi a seguinte:
...
Em termos globais, e até ao ano de 2020, a evolução da taxa anual de mortalidade infantil evidenciou uma tendência de decréscimo. No entanto, a partir de 2021 a tendência inverteu-se e, desde então, as taxas anuais de mortalidade infantil têm vindo a aumentar. 
...
```{r ediq-mapa, out.width="50%", fig.align='center'}
# mapa EDI
...
### Riscos relativos: qualidade do ajustamento M1-M7 {#sec-ResultadosRR}

Para os modelos ajustados (M1-M7) com priors especificadas na secção @sec-MetodosPriors apresentam-se na tabela seguinte os resultados obtidos com as medidas de ajustamento Deviance Information Criterion (DIC) e Watanabe-Akaike Information Criterion (WAIC).
...
### Riscos relativos: M1 {#sec-ResultadosM1}
...
Será consensual considerar que um dos problemas de M1 é o de ser linear e não conseguir captar a tendência de aumento acentuado observado nos últimos anos. Nas secções seguintes serão analisados os modelo M2 e M6, onde se adicionam a componente espacial e temporal para avaliar o impacto na variação da mortalidade infantil observada.

### Riscos relativos: M2 {#sec-ResultadosM2}

No modelo M2 ajustou-se um efeito aleatório das componentes espacial - $\zeta_i = u_i+v_i$ - e os efeitos global - $\beta$ - e local - $\delta_i$ - da componente temporal. 
...
O parâmetro $\delta_i$ representa a interação entre o tempo e o espaço (i.e. do concelho $i$), ou seja,  a diferença entre a tendência temporal global ($\beta$) e a tendência temporal específica de cada concelho.
...
O mapa seguinte ilustra as estimativas obtidas para $\exp(\delta_i)$ com o modelo M2, em cada concelho. As estimativas apresentadas referem-se à média da distribuição à posteriori de $\exp(\delta_i)$ obtida em cada concelho.

```{r m2-mapa-delta-pt, fig.align='center', out.width="50%"}

d_gis <- pt_gis
d_gis$delta <- delta

dplot <- ggplot(d_gis) +
  geom_sf(aes(fill = delta), color = "grey80", size = 0.1) +
  scale_fill_gradient2(
    low = "#2166AC", 
    mid = "white", 
    high = "#B2182B",
...
A função exponencial aplicada a $\delta_i$ representa o efeito (multiplicativo) do concelho $i$ em relação à tendencia temporal média de mortalidade infantil esperada para o país.

Os valores mais elevados tendem a estar concentrados em algumas áreas da regiões centro e sul de Portugal Continental. Para identificar concelhos com tendências temporais significativas, obtiveram-se intervalos de credibilidade a 95% para $\exp(\delta_i)$. Esta quantidade representa o fator de variação anual do RR de mortalidade infantil, ou seja, a razão dos RRs entre anos consecutivos. O gráfico seguinte apresenta estes intervalos de credibilidade por concelho, ordenados de forma crescente.

```{r deltaplot, fig.align='center', out.width="50%"}

# intercept
beta_global <- m2_res$summary.fixed["idtime", ]
...
df_deltai <- data.frame(
  idarea = delta_i$ID,
  concelho = unique(d$cc)[delta_i$ID],
  linf95 = exp(delta_i$`0.025quant`),
  lmed95 = exp(delta_i$`0.5quant`),
  lsup95 = exp(delta_i$`0.975quant`)
) %>%
  arrange(lmed95)  # ordenar por mediana

# plot concelhos
ggplot(df_deltai, aes(x = reorder(concelho, lmed95), y = lmed95)) +
  geom_errorbar(aes(ymin = linf95, ymax = lsup95, color =ifelse(linf95 >= 1, "*", "n.s.")), 
                width = 0.4,
                alpha = 0.7) +
  geom_point(aes(color = ifelse(linf95 >= 1, "*", "n.s.")), size = 1.5) +
  geom_text_repel(data = subset(df_deltai, linf95 >= 1),
                    aes(label = concelho), 
                    size = 2.5,
                    color = "black",
                    box.padding = 0.3,
  ...
  labs(
    x = "Concelho",
    y = "exp(\u03B4\u1D62)",
    title = "Tendências temporais do RR por concelho (M2)",
    subtitle = "Intervalos de credibilidade de 95%"
  ) +
...
Partindo deste gráfico podemos observar que todos os concelhos apresentam intervalos que incluem o valor 1, indicando ausência de tendências temporais locais significativamente diferentes da tendência nacional. Apesar disso, pode observar-se que os concelhos com um aumento mais acentuado, estão maioritariamente concentrados na região da grande Lisboa. 

```{r m2-mapa-zoom-delta-pt, fig.align='center', out.width="50%"}

mun_labs_delta <- d_gis %>%
  filter(delta > 1.02) %>%
  st_centroid()

dplot_zoom <- ggplot() +
  geom_sf(data = d_gis, aes(fill = delta), color = "grey80", size = 0.1) +
  geom_sf_text(
    data = mun_labs_delta,
    aes(label = nome_cc),
    size = 2.5,
  ...
  labs(
    title = "Tendência temporal [exp(\u03B4\u1D62)]",
    subtitle = "Modelo M2"
  ) +
...
A figura ilustra a distribuição de $\exp(\delta_i$ na Grande Lisboa. Apenas cinco concelhos em todo o país apresentam tendência temporal superior a 1.02, dos quais quatro se localizam nesta região e encontram-se identificados no mapa; o quinto é Albufeira.

Por questões de simplicidade, apresentam-se na secção seguinte apenas os resultados do modelo M6. Esta escolha prende-se com o facto dos modelos M3-M7 apresentaram resultados de ajustamento similares e porque, entre os modelos avaliados, M6 foi o modelo que devolveu os melhores resultados com DIC. 

### Riscos relativos: M6 {#sec-ResultadosM6}

O modelo M6 inclui o efeito espacial dado pela componentes espacial estruturada $u_i$ e não-estruturada $v_i$, o efeito temporal dado pela componente estruturada $\gamma_j$ (definida por um passeio aleatório de primeira ordem conforme apresentado em @sec-MetodosModelosBayes), e não-estruturada $\phi_j$ com distribuição $\text{N}\sim (0,\sigma_\phi^2)$, e o efeito da covariável EDI.
...
```{r mapas-rr, fig.align='center', out.width="50%"}
...
### Riscos relativos em 2024: M6 {#sec-ResultadosM62024} 

Nesta secção apresentam-se os resultados para 2024 com mais detalhe. Em particular apresenta-se um gráfico de barras com as estimativas do RR em cada concelho e o respectivo IC95%. As barras dos concelhos com RR significativamente superiores (a 1) estão pintadas a vermelho. Ilustram-se também os padrões espaciais com os mapas de RR e IC95% para 1) Portugal e 2) região de Lisboa. O gráfico de barras evidência os RRs com estimativas significativemnte superiores a 1 no ano de 2024: Amadora, Barreiro e Odemira.

```{r barplot, eval=T,fig.align='center', out.width="50%"}
...
Os padrões espaciais do RR em Portugal, ampliando depois o mapa para visualizar com mais detalhe as regiões de Lisboa (que inclui concelhos densamente povoados) e a região do Sul. A par dos mapas de RR apresentam-se os mapas dos limites inferior (LInf) e superior (LSup) do IC95%.

```{r mapa-RR2024, fig.align='center', out.width="50%", eval =FALSE}
...
saveRDS(m1_res, file = "mapa_R1.rds")
