...
Os concelhos com um aumento mais acentuado, estão maioritariamente concentrados na região da grande Lisboa. A figura seguinte ilustra a distribuição de $\exp(\delta_i)$ na Grande Lisboa. Apenas cinco concelhos em todo o país apresentam tendência temporal superior a 1.02, dos quais quatro se localizam nesta região e encontram-se identificados no mapa; o quinto é Albufeira.
...
# plot concelhos
deltaplot <- ggplot(df_deltai, aes(x = reorder(concelho, lmed95), y = lmed95)) +
...
Para identificar concelhos com tendências temporais significativas, obtiveram-se intervalos de credibilidade a 95% para $\exp(\delta_i)$. Esta quantidade representa o fator de variação anual do RR de mortalidade infantil, ou seja, a razão dos RRs entre anos consecutivos. O gráfico seguinte apresenta estes intervalos de credibilidade por concelho, ordenados de forma crescente.

```{r deltabars, fig.align='center', out.width="50%"}
deltaplot
```
...
```{r tempoplot,  fig.align='center', out.width="50%"}
