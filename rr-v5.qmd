...
abstract: |
  O objetivo desta análise é avaliar a evolução espacial e temporal da mortalidade infantil em Portugal ao longo da última década.
...
Para analisar a evolução espaço-temporal da mortaliade infantil em Portugal desde 1 de janeiro de 2015 a 31 de dezembro de 2024, foram usados os dados seguintes:
...
casos <- read.table("./data/casos.txt", sep = "\t", header = T, encoding = "latin1")
# retirar acores e madeira
casos <- casos %>% filter(Nomes_cc %in% pk)
names(casos) <- c("cc", 2015:2024)

pop <- read.table("./data/pop.txt", sep = "\t", header = T, encoding = "latin1")
# retirar acores e madeira
...
```{r dados-edi}
edi <- read.table("./data/edi_2011.csv", sep = ";", dec = ",", header = T, encoding = "latin1")
...
Para estimar o risco relativo (RR) de mortalidade infantil por concelho (em relação ao da população de referência), usou-se uma abordagem Bayesiana baseada numa regressão ecológica espaço-temporal. Esta abordagem permitiu atenuar o efeito dos pequenos números nas estimativas do RR, provocado por concelhos com baixo número de nados-vivos (e consequentemente óbitos) e, ao mesmo tempo, identificar padrões espaço-temporais do RR de morrer durante o primeiro ano de vida. Em algumas das regressões especificadas adicionou-se a covariável EDI, que permitiu avaliar o impacto dos efeitos aleatórios espaciais e temporais na variação do RR, controlando o efeito do índice EDI. 

Com recurso ao package INLA^[https://doi.org/10.1111/j.1467-9868.2008.00700.x] especificou-se uma regressão Poisson:  

$$
Y_{ij}\sim \text{P}\big(E_{ij}\theta_{ij}\big)
$$

onde $Y_{ij}$ o número de óbitos observados no concelho $i$ no ano $j$, $E$ representa o número de óbitos esperados se a taxa de mortalidade infantil fosse a da população de referência e $\theta$ representa o RR de morrer durante o primeiro ano de vida. 

Exploraram-se diferentes especificações para modelar $\theta_{ij}$ (em vez de $\theta_{ij}$ modelou-se $\log(\theta_{ij})$ por causa da relação linear com os preditores): 
...
Para a tendência global especificou-se uma distribuição gaussiana não-informativa de média nula e precisão $\tau= 1/\sigma^2=$ `control.fixed$prec.intercept`.
$$
\alpha \sim N(`r control.fixed$mean.intercept`, `r 1/control.fixed$prec.intercept`)
$$

Para o parãmetro associado ao índice EDI especificou-se uma prior informativa com distribuição gaussiana:  
$$
\beta_{EDI} \sim N(`r control.fixed$mean`, `r 1/control.fixed$prec`)
$$
assumindo à priori a expectativa de uma associação positiva entre a privação socio-económica e a mortalidade infantil.

#### Efeitos aleatórios

As distribuições a priori para os hiperparâmetros dos efeitos aleatórios foram especificadas usando priors de complexidade penalizada (PC priors). 
...
Para a precisão marginal desta compontente especificou-se uma distribuição $\tau_\varepsilon \sim \text{Gamma}(a,b)$ com parâmetros $a,b$ que favorecem uma baixa variabilidade espacial no contecto da mortalidade infantil.

Para o parâmetro associado à autocorrelação espacial especificou-se uma distribuição $\text{Beta} \sim (\alpha,\beta)$ com parâmetros $\alpha,\beta$ assumindo que à priori, a componente não-estruturada da variabilidade espacial é dominante (sobre a estruturada). Os hiperparâmetros associados são determinados pela condição:

$$
P(\phi> `r hyper.bym2$phi$param[2]`)=`r hyper.bym2$phi$param[2]`
$$

Para a estrutura temporal especificou-se um passeio aleatório (*random walk*, RW1) de primeira ordem com erros aleatórios gaussianos $\varepsilon \sim \text{N}(0,\sigma^2)$ e uma precisão elevada (i.e. desvio padrão reduzido):

$$
P(\sigma>`r hyper.temporal$prec$param[1]`)=`r hyper.temporal$prec$param[2]`
$$

Para a precisão desta componente especificou-se a distribuição $\tau_\varepsilon \sim \text{Gamma}(a,b)$ com parâmetros $a,b$ que favorecem uma evolução temporal suave, i.e., com probabilidade de 1% de exceder 0.1 (i.e. variação anual do risco relativo entre $-10\%$ e $11\%$:
...
```{r m2-mapas-pt, fig.cap="Médias da distribuição à posteriori para o efeito aleatório da componentes espacial (esq.) e para efeito do tempo em cada local (dir.)", fig.align='center', out.width="50%"}
d_gis <- pt_gis
...
No mapa de $\zeta$ (componente espacial) identifica-se uma tendência para os efeitos serem mais elevados na região de Lisboa e região Sul. Na distribuição de $\exp(\delta_i)$ identifica-se um aumento dos RR entre anos consecutivos mais acentuado em alguns concelhos destas regiões. Os mapas seguintes apresentam com mais detalhe os mapas de $\zeta$ e $\exp(\delta_i)$ para a região de Lisboa. 
